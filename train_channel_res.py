...
def train(args):
    ...
    for epoch_num in tqdm(range(args.num_epochs)):
        ...
        for i, (cqcc, cqcc_chan, audio_fn, tags, labels, channels) in enumerate(tqdm(trainDataLoader)):
            cqcc = cqcc.transpose(2,3).to(args.device)
            cqcc_chan = cqcc_chan.transpose(2,3).to(args.device)
            ...
            feats_chan, cqcc_chan_outputs = cqcc_model(cqcc_chan)

            cqcc_loss = criterion(cqcc_outputs, labels)
            cqcc_loss += criterion(cqcc_chan_outputs, labels)
